...
describe("CertificacionCredenciales", function () {
  ...
  it("debe autorizar al owner como emisor inicial", async function () {
    const { contrato, owner } = await desplegarContrato();

    const estaAutorizado = await contrato.emisoresAutorizados(owner.address);

    expect(estaAutorizado).to.equal(true);
  });

  it("debe permitir al owner autorizar un nuevo emisor", async function () {
    const { contrato, emisor } = await desplegarContrato();

    await contrato.autorizarEmisor(emisor.address, true);
  ...
  it("no debe permitir emitir dos veces el mismo certificado", async function () {
    const { contrato } = await desplegarContrato();

    const codigo = "CERT-005";
    const hashCertificado = ethers.keccak256(ethers.toUtf8Bytes("certificado duplicado"));
    const hashTitular = ethers.keccak256(ethers.toUtf8Bytes("titular duplicado"));

    await contrato.emitirCertificado(codigo, hashCertificado, hashTitular);

    await expect(
      contrato.emitirCertificado(codigo, hashCertificado, hashTitular)
